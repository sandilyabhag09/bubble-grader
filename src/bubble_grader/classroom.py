...
RESULTS_TOPIC = "Graded results"


def ensure_topic(email: str, course_id: str, name: str = RESULTS_TOPIC) -> str | None:
    """Return the id of the Classwork topic called ``name``, creating it if
    missing. Returns None if topics can't be read/created (missing scope) —
    the post then lands untopiced rather than failing the whole return."""
    def compute():
        svc = service_for(email, "classroom", "v1")
        topics = _paginate(
        ...
        for t in topics:
            if (t.get("name") or "").strip().lower() == name.lower():
                return [t["topicId"]]
        created = svc.courses().topics().create(courseId=course_id, body={"name": name}).execute()
        return [created["topicId"]]
    try:
        return _cached(("topic", email, course_id, name), TTL_COURSEWORK, compute)[0]
    except Exception:
        return None


def create_student_material(
    email: str,
    course_id: str,
    title: str,
    description: str,
    *,
    ...
    """Post a Classwork *material* visible only to the given students (and teachers).

    This is how graded work gets 'returned with a marked-up sheet': Classroom's
    API doesn't let a teacher attach files to a STUDENT's submission, and an
    announcement would be a Stream post per student. A material assigned to
    INDIVIDUAL_STUDENTS sits in the Classwork tab (under ``topic_id``) and, with
    the class setting "Classwork on the stream: Hide notifications", never
    touches the Stream. Needs the classroom.courseworkmaterials scope.
    """
    body: dict = {
        "title": title[:3000],
    ...
    svc = service_for(email, "classroom", "v1")
    return svc.courses().courseWorkMaterials().create(courseId=course_id, body=body).execute()
